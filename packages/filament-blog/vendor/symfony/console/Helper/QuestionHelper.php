...
class QuestionHelper extends Helper
{
    ...
    private function doAsk($inputStream, OutputInterface $output, Question $question): mixed
    {
        ...
        if ($autocomplete === null || ! self::$stty || ! Terminal::hasSttyAvailable()) {
            $ret = false;
    ...
    {
        if ($this->getHelperSet() !== null && $this->getHelperSet()->has('formatter')) {
    ...
    private function autocomplete(OutputInterface $output, Question $question, $inputStream, callable $autocomplete): string
    {
        ...
        while (! feof($inputStream)) {
            while ($isStdin && @stream_select($r, $w, $w, 0, 100) === 0) {
            ...
            if ($c === false || ($ret === '' && $c === '' && $question->getDefault() === null)) {
                shell_exec('stty '.$sttyMode);
                throw new MissingInputException('Aborted.');
            } elseif ($c === "\177") { // Backspace Character
                if ($numMatches === 0 && $i !== 0) {
                    $i--;
    ...
    private function cloneInputStream($inputStream)
    {
        ...
        if ($seekable === true && ! \in_array($mode, ['r', 'rb', 'rt'])) {
