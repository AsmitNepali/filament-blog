...
abstract class AttributeClassLoader implements LoaderInterface
{
        ...
        foreach ($method->getParameters() as $param) {
            ...
            foreach ($paths as $locale => $path) {
                if (preg_match(sprintf('/\{%s(?:<.*?>)?\}/', preg_quote($param->name)), $path)) {
                    if (\is_scalar($defaultValue = $param->getDefaultValue()) || $defaultValue === null) {
    ...
    {
        return \is_string($resource) && preg_match('/^(?:\\\\?[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)+$/', $resource) && (! $type || $type === 'attribute');
    }
    ...
    protected function getGlobals(\ReflectionClass $class): array
    {
        ...
        if ($attribute = $class->getAttributes($this->routeAnnotationClass, \ReflectionAttribute::IS_INSTANCEOF)[0] ?? null) {
            ...
            if ($annot->getPath() !== null) {
                $globals['path'] = $annot->getPath();
            }

            $globals['localized_paths'] = $annot->getLocalizedPaths();

            if ($annot->getRequirements() !== null) {
                $globals['requirements'] = $annot->getRequirements();
            }

            if ($annot->getOptions() !== null) {
                $globals['options'] = $annot->getOptions();
            }

            if ($annot->getDefaults() !== null) {
                $globals['defaults'] = $annot->getDefaults();
            }

            if ($annot->getSchemes() !== null) {
                $globals['schemes'] = $annot->getSchemes();
            }

            if ($annot->getMethods() !== null) {
                $globals['methods'] = $annot->getMethods();
            }

            if ($annot->getHost() !== null) {
                $globals['host'] = $annot->getHost();
            }

            if ($annot->getCondition() !== null) {
