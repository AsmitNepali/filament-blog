...
class CacheAttributeListener implements EventSubscriberInterface
{
    ...
    public function onKernelResponse(ResponseEvent $event): void
    {
        ...
        foreach (array_reverse($attributes) as $cache) {
            if ($cache->smaxage !== null && ! $response->headers->hasCacheControlDirective('s-maxage')) {
            ...
            if ($cache->maxage !== null && ! $response->headers->hasCacheControlDirective('max-age')) {
                $response->setMaxAge($this->toSeconds($cache->maxage));
            }

            if ($cache->maxStale !== null && ! $response->headers->hasCacheControlDirective('max-stale')) {
                $response->headers->addCacheControlDirective('max-stale', $this->toSeconds($cache->maxStale));
            }

            if ($cache->staleWhileRevalidate !== null && ! $response->headers->hasCacheControlDirective('stale-while-revalidate')) {
                $response->headers->addCacheControlDirective('stale-while-revalidate', $this->toSeconds($cache->staleWhileRevalidate));
            }

            if ($cache->staleIfError !== null && ! $response->headers->hasCacheControlDirective('stale-if-error')) {
