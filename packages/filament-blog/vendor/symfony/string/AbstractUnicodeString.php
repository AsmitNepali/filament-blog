...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    public function ascii(array $rules = []): self
    {
        ...
        while (\strlen($s) - 1 > $i = strspn($s, self::ASCII)) {
            ...
            if ($rule instanceof \Transliterator) {
                ...
                } elseif ($rule === '[:nonspacing mark:] remove') {
                    $s = preg_replace('/\p{Mn}++/u', '', $s);
                } elseif ($rule === 'latin-ascii') {
                    $s = str_replace(self::TRANSLIT_FROM, self::TRANSLIT_TO, $s);
                } elseif ($rule === 'de-ascii') {
    ...
    public function camel(): static
    {
        ...
        $str->string = str_replace(' ', '', preg_replace_callback('/\b.(?![A-Z]{2,})/u', static function ($m) {
            ...
            return ++$i === 1 ? ($m[0] === 'İ' ? 'i̇' : mb_strtolower($m[0], 'UTF-8')) : mb_convert_case($m[0], \MB_CASE_TITLE, 'UTF-8');
    ...
    public function join(array $strings, ?string $lastGlue = null): static
    {
        ...
        $tail = $lastGlue !== null && \count($strings) > 1 ? $lastGlue.array_pop($strings) : '';
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0): array
    {
        ...
        try {
            if ($match($regexp.'u', $this->string, $matches, $flags | \PREG_UNMATCHED_AS_NULL, $offset) === false) {
    ...
    {
        if ($chars !== " \t\n\r\0\x0B\x0C\u{A0}\u{FEFF}" && ! preg_match('//u', $chars)) {
    ...
    private function wcswidth(string $string): int
    {
        ...
        foreach (preg_split('//u', $string, -1, \PREG_SPLIT_NO_EMPTY) as $c) {
            ...
            if ($codePoint === 0 // NULL
                ...
                || ($codePoint >= 0x200B && $codePoint <= 0x200F) // ZERO WIDTH SPACE to RIGHT-TO-LEFT MARK
                || $codePoint === 0x2028 // LINE SEPARATOR
                || $codePoint === 0x2029 // PARAGRAPH SEPARATOR
                || ($codePoint >= 0x202A && $codePoint <= 0x202E) // LEFT-TO-RIGHT EMBEDDING to RIGHT-TO-LEFT OVERRIDE
                || ($codePoint >= 0x2060 && $codePoint <= 0x2063) // WORD JOINER to INVISIBLE SEPARATOR
            ) {
                continue;
            }

            // Non printable characters
            if ($codePoint < 32 // C0 control characters
                || ($codePoint >= 0x07F && $codePoint < 0x0A0) // C1 control characters and DEL
            ) {
                return -1;
            }
