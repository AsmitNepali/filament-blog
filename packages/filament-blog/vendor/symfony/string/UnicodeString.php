...
class UnicodeString extends AbstractUnicodeString
{
    ...
    {
        $str = clone $this;
        $str->string = $this->string.(\count($suffix) <= 1 ? ($suffix[0] ?? '') : implode('', $suffix));
    ...
    public function endsWith(string|iterable|AbstractString $suffix): bool
    {
        ...
        $form = $this->ignoreCase === null ? \Normalizer::NFD : \Normalizer::NFC;
        ...
        if ($this->ignoreCase) {
            return mb_stripos(grapheme_extract($this->string, \strlen($suffix), \GRAPHEME_EXTR_MAXBYTES, \strlen($this->string) - \strlen($suffix)), $suffix, 0, 'UTF-8') === 0;
        }
    ...
    public function equalsTo(string|iterable|AbstractString $string): bool
    {
        ...
        if ($string !== '' && $string !== false && $this->ignoreCase) {
    ...
    public function split(string $delimiter, ?int $limit = null, ?int $flags = null): array
    {
        ...
        while ($limit > 1 && false !== $i = $indexOf($tail, $delimiter)) {
    ...
    public function startsWith(string|iterable|AbstractString $prefix): bool
    {
        ...
        if ($this->ignoreCase) {
            return mb_stripos(grapheme_extract($this->string, \strlen($prefix), \GRAPHEME_EXTR_MAXBYTES), $prefix, 0, 'UTF-8') === 0;
        }
