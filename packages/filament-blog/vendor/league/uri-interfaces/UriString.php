...
final class UriString
{
    ...
    public static function parse(Stringable|string|int $uri): array
    {
        ...
        if (preg_match(self::REGEXP_INVALID_URI_CHARS, $uri) === 1) {
        ...
        if ($parts['scheme'] === ':' || preg_match(self::REGEXP_URI_SCHEME, $parts['scontent']) !== 1) {
            throw new SyntaxError(sprintf('The uri `%s` contains an invalid scheme', $uri));
        }

        if ('' === $parts['scheme'].$parts['authority'] && preg_match(self::REGEXP_INVALID_PATH, $parts['path']) === 1) {
    ...
    private static function filterRegisteredName(string $host): string
    {
        ...
        if (preg_match(self::REGEXP_REGISTERED_NAME, $formattedHost) === 1) {
            return $host;
        }

        //to test IDN host non-ascii characters must be present in the host
        if (preg_match(self::REGEXP_IDN_PATTERN, $formattedHost) !== 1) {
    ...
    private static function isIpHost(string $ipHost): bool
    {
        ...
        if (preg_match(self::REGEXP_IP_FUTURE, $ipHost, $matches) === 1) {
            return ! in_array($matches['version'], ['4', '6'], true);
        }

        $pos = strpos($ipHost, '%');
        if ($pos === false || preg_match(self::REGEXP_INVALID_HOST_CHARS, rawurldecode(substr($ipHost, $pos))) === 1) {
            return false;
        }

        $ipHost = substr($ipHost, 0, $pos);

        return filter_var($ipHost, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) !== false
