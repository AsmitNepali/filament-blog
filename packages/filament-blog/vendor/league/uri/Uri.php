...
final class Uri implements UriInterface
{
    ...
    {
        if ($scheme === null || array_key_exists($scheme, self::SCHEME_DEFAULT_PORT)) {
            return $scheme;
        }

        $formattedScheme = strtolower($scheme);
        if (array_key_exists($formattedScheme, self::SCHEME_DEFAULT_PORT) || preg_match(self::REGEXP_SCHEME, $formattedScheme) === 1) {
    ...
    private function formatHost(?string $host): ?string
    {
        ...
        if (count($formattedHostCache) > self::MAXIMUM_FORMATTED_HOST_CACHED) {
    ...
    private function formatIp(string $host): string
    {
        ...
        if (preg_match(self::REGEXP_HOST_IPFUTURE, $ip, $matches) === 1 && ! in_array($matches['version'], ['4', '6'], true)) {
            return $host;
        }
        ...
        if (preg_match(self::REGEXP_HOST_GEN_DELIMS, rawurldecode(substr($ip, $pos))) === 1) {
            throw new SyntaxError('The host `'.$host.'` is invalid : the IP host is malformed.');
        }
    ...
    {
        return match (true) {
            $this->scheme === 'data' => Encoder::encodePath(self::formatDataPath($path)),
            $this->scheme === 'file' => $this->formatFilePath(Encoder::encodePath($path)),
    ...
    {
        if (preg_match(self::REGEXP_MIMETYPE, $mimetype) !== 1) {
    ...
    {
        [$user, $pass] = $this->userInfo !== null ? explode(':', $this->userInfo, 2) : [null, null];
