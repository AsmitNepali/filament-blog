 ...
 */
class BaseUri implements JsonSerializable, Stringable, UriAccess
{
    ...
    final protected function computeOrigin(Psr7UriInterface|UriInterface $uri, ?string $nullValue): Psr7UriInterface|UriInterface|null
    {
        ...
        if ($uri instanceof Psr7UriInterface) {
            ...
            $components = array_map(fn ($component) => $component === null ? '' : $component, $components);
        }

        return match (true) {
            $components['scheme'] !== null && isset(static::WHATWG_SPECIAL_SCHEMES[strtolower($components['scheme'])]) => $uri
                ->withFragment($nullValue)
                ->withQuery($nullValue)
                ->withPath('')
    ...
    final protected function normalize(Psr7UriInterface|UriInterface $uri): string
    {
        ...
        $query = $uri->getQuery();
        $pairs = $query === null ? [] : explode('&', $query);
        sort($pairs);
     ...
     */
    final protected static function filterUri(Stringable|string $uri, ?UriFactoryInterface $uriFactory = null): Psr7UriInterface|UriInterface
    {
        return match (true) {
    ...
    final protected function resolvePathAndQuery(Psr7UriInterface|UriInterface $uri): array
    {
        ...
        if ($targetPath === '') {
            ...
            if ($this->uri->getAuthority() !== null && ! str_starts_with($targetPath, '/')) {
        ...
        $basePath = $this->uri->getPath();
        if ($this->uri->getAuthority() !== null && $basePath === '') {
            $targetPath = '/'.$targetPath;
        }
    ...
    final protected static function formatHost(Psr7UriInterface|UriInterface $uri): Psr7UriInterface|UriInterface
    {
        ...
        return match (true) {
            $converted !== null => $uri->withHost($converted),
            $host === '',
            $uri instanceof UriInterface => $uri,
